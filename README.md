...
- **拖拽窗口** — 双击安装副本打开窗口，批量拖入文件夹复制；可选驻留托盘
...
- **双击任意 `copy-tree.exe`** — 打开拖拽窗口；若尚未安装且没有跳过过安装引导，会先弹一次安装询问（见下）
- **开始菜单「copy-tree」** — 直接打开已安装副本的拖拽窗口

### 首次安装引导

首次双击且尚未安装时，会弹一次「是否安装 copy-tree 右键菜单」：

- 选「是」— 复制到安装位置并注册右键菜单，之后双击直接开窗
- 选「否」（暂不）— 记住选择，之后双击不再询问、直接开窗；窗口顶部横幅保留「安装右键菜单」按钮，想装时随时点它

### 窗口顶部横幅

横幅只在安装状态需要处理时出现，各状态含义：

| 横幅提示 | 含义 | 按钮 |
|----------|------|------|
| 尚未安装右键菜单 | 还没安装，装好才能右键使用 | 安装右键菜单 |
| 检测到新版本 … | 下载目录里的 exe 比已安装副本新，可就地更新 | 更新到 v… |
| 本机已安装更新版本 … | 已安装副本比手里的 exe 新（版本回退），右键菜单不受影响 | 卸载已装副本… |
| 安装副本丢失或损坏 | 右键菜单指向的程序不存在了，需要重装修复 | 重新安装 |
| 检测到旧安装路径 … | 旧版本装在非标准位置，迁移后右键菜单不再依赖下载目录 | 迁移到标准位置 |

### 右键菜单与安装副本的关系

安装时程序会把自身复制到 `%LOCALAPPDATA%\copy-tree\`（安装副本），右键菜单始终指向这份持久副本。因此**删除下载目录里的 `copy-tree.exe` 不影响右键菜单**；要更新时，用新版 exe 双击即可覆盖安装副本。

### 卸载入口

- 拖拽窗口右下角「卸载 copy-tree…」按钮
- 开始菜单「卸载 copy-tree」快捷方式
...
安装后自动复制到 `%LOCALAPPDATA%\copy-tree\`（安装副本），右键菜单指向该副本。再次双击已安装副本会打开拖拽窗口；用新版 exe 双击可更新，卸载入口在拖拽窗口和开始菜单的「卸载 copy-tree」快捷方式。
